...
class VerTareasHoy extends StatefulWidget {
  const VerTareasHoy({Key? key}) : super(key: key);

  @override
  State<VerTareasHoy> createState() => _VerTareasHoyState();
}

class _VerTareasHoyState extends State<VerTareasHoy> {
  List<Tarea> tareas =[];

  void actualizarLista() async{
    List<Tarea> temporal = await DB.mostrarTarea();
    setState(() {
      tareas=temporal;
    });
  }
